...
if (isset($_POST['submit'])){//判断是否点击
    if (is_uploaded_file($_FILES['upfile']['tmp_name'])){//判断是否是hhtp post上传
        ...
        $houzhui=pathinfo($_FILES['upfile']['name']);//houzui[extension]是文件后缀名
...
if (isset($_POST['dianji'])){
    if (is_uploaded_file($_FILES['myfile']['tmp_name'])){
        $newname=date('Y-m-d-H-i-s').rand(1000,3000);
        $houzhui=pathinfo($_FILES['myfile']['name']);//后缀获取name的而不是tmp_name的
...
if (isset($_POST['submit'])){
    if (is_uploaded_file($_FILES['file']['tmp_name'])){
        $newname=date('Y-m-d H-i-s' ).rand(1000,9999);
        $houzhui=pathinfo($_FILES['file']['name']);
        if (move_uploaded_file($_FILES['file']['tmp_name'],"update/{$newname}.{$houzhui['extension']}")){
            echo '上传成功';
        }
        else{
            echo '上传失败';
